...
#                key1=tag1+'_'+tag
    ...
    def comp_candidate_order_mix(self, pcd, tfd, a_users_tag_dict,ruledict,removelist):
        ...
        for tag in a_users_tag_dict:
            ...
            print pcd[tag],tag
            tag1=pcd[tag]['p'][0]
            if tag1 not in tag and tag1!='ROOT' and tag1 not in tlist:# and tag1 not in a_users_tag_dict:
                #key=tag+'_'+tag1
                #key1=tag1+'_'+tag
                value0=pcd[tag]['p'][1]
                value1=tfd[tag1]
            ...
            tag2=pcd[tag1]['c']
            for key,value in tag2.items():
                if key!=tag and key not in tag and key not in tlist: #and key not in a_users_tag_dict:
                    k=tag+'_'+key
                    k1=key+'_'+tag
                    sim=ruledict.get(k,0)+ruledict.get(k1,0)
                    value1=tfd[key]
            ...
            if 'c' in pcd[tag]:
                for key,value in pcd[tag]['c'].items():
                    if key not in tag and key not in tlist:# and key not in a_users_tag_dict:
                        value1=tfd[key]
