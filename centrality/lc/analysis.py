...
class Locallist(object):
    ...
    def check_rulelist(self,pairs):
        ...
        for loc in name:
            infile=open('./rulelist_tfidf_new_'+loc,'r')
            for line in infile:
                #print line,loc
    ...
    def tag_pair_distance_at_different_location(self,outfile,eg):
        outfile=open(outfile,'w')
        tagpair1=[['basketball','rocket','laker','maverick','heat','warrior','bull'],['oil','green'],['business','stock','investment'],['olympic','rugby'],['coding']]
        tagpair2=[['nba'],['energy'],['finance'],['athlete'],['dev']]
        pairs = self.construct_pair(tagpair1[eg],tagpair2[eg])
        print pairs
...
if __name__=='__main__':
    ...
    eg=3
    l.tag_pair_distance_at_different_location('tagpair_eg'+str(eg),eg)
