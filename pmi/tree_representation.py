...
class build_tree(object):
    ...
    def check_ring(self,key,value,gpd_bb,gpd_bb_old):
        ...
        return False
        #print exist
#find how the nodes are distributed in the tree. branching factor    
    def structural_eval(self,tree):
        ...
        while tmplvl !=[]:
            ...
            for key in tmplvl:
                for item in tree[key]['out']:
                    new_lvl.append(item)
                if len(tree[key]['out'])!=0:
                    branching.append(len(tree[key]['out']))
            tmplvl=new_lvl
            lvl.append(tmplvl)
        total_num=0    
        for lvs in lvl:
        ...
        return [percenta,percenta1,aut,aut1,b]
...
def main2():
    ...
    gpd=open(gpd_file,'r')
    gpd=cjson.decode(gpd.readline())
    tree=build_tree()
    inoutdict=tree.tree_from_gpd(gpd)
    t=tree.structural_eval(inoutdict)
    outfile=open('structure-cost-hou','w')
    outfile.write(cjson.encode(t))
