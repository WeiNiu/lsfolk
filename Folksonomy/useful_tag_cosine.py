...
#cosine
infile2='/spare/wei/local/cosine_ch3'

#selected user_tag_dict
infile3='/spare/wei/local/user_tag_dict_ch3'
outfile=open('newtag_cosine','w')
infile2=open(infile2,'r')
cosinedict=cjson.decode(infile2.readline())
new_cosine=defaultdict(dict)
for key,value in cosinedict.iteritems():
    key1,key2=key.split('_')
    new_cosine[key1][key2]=value
    new_cosine[key2][key1]=value
...
for line in open(infile3,'r'):
    ...
    for tag,value in closest3[:int(length/10)]:
        if tag not in new_cosine.keys():
            continue
        else:
            order=sorted(new_cosine[tag].items(),key=itemgetter(1),reverse=1)
            for item in order:
                if item[1]>0.5 and item[0] not in currenttag:
