...
#args rootdict  unitags    gpd    folk
...
graph_parent_dict={}
graph_parent_dict_s={}
removelistf=open(sys.argv[5],'r')
removelist=cjson.decode(removelistf.readline())
for line in infile:
...
#        G.add_edge('ROOT',node)
    #if  root_dict[1][0][1]>0.25:
    if  root_dict[1][0][1]>0.4:
        print root_dict[0][-4:],root_dict[0]
        graph_parent_dict[root_dict[0]]=root_dict[1][0][0]
        graph_parent_dict_s[root_dict[0]]=root_dict[1][0]
    else:
        graph_parent_dict[root_dict[0]]='ROOT'
        graph_parent_dict_s[root_dict[0]]=['ROOT',0]
for tag in taglist:
    
    if tag not in removelist and tag not in graph_parent_dict.keys() and tag[-4:]!=' and' and tag[-3:]!='of':
        graph_parent_dict[tag]='ROOT'
        graph_parent_dict_s[tag]=['ROOT',0]
...
outfile3.write(cjson.encode(graph_parent_dict_s))
