'''prediction for farely new user.
'''
            ...
            while found==False:
                for item in utd1:
                    if item[0] in rule_by_tag:
                        ...
                        for rule in rules:
                            if rule[1] not in user_tag_dict:
                                #print user,rule[1]
                                cnt+=1
    ...
    def prediction_by_strong_rule(self,rulelistfile,ground_truth_file,user_tag_dict_file):
        ...
        for line in open(rulelistfile,'r'):
            ...
            if line[2]>0.5:
                rule_by.append(line)
        #for key in rule_by:
        rule_by=sorted(rule_by,key=itemgetter(2),reverse=1)
        cnt=0
        cntt=0
        cntp=0
        for user,utd in user_tag_dict.iteritems():
            ...
            while found==False:
                for item in utd1:
                    if item[0] in rule_by:
                        rules=rule_by[item[0]]
                        for rule in rules:
                            if rule[1] not in user_tag_dict and rule[2]:
                                #print user,rule[1]
                                cnt+=1
                                if rule[1] in ground_truth[user]:
                                    cntt+=1
...
if __name__ =='__main__':
    ...
    #data.nfold_(4,docs)
    data.prediction_by_strong_rule_of_most_voted_tag('../rulelist3_houston','user_tag_dict_houston1_fold1','user_tag_dict_houston1_train_1')
