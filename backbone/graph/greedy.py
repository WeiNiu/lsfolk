...
def reduce_graph(graph,edges):
    for node in graph:
        if len(graph[node]['in'])<3:
            ...
            outlist=sorted(graph[node]['in'].items(),key=itemgetter(1))[:-2]
    ...
    while len(edges)>numnode:
        ...
        for edge in edges:
            ...
            if len(graph[edge[1]]['in'])>=2:
                ...
                print edge
                hops,trail=shortest_weight(cur_graph,edge[0],edge[1])
                print trail
                cost=trail[-1]
                #print hops,trail
                #cost=(hops-1)*edge[2]**2
                cost=abs(edge[2]-cost)
                print 'cost',cost
        ...
        for rml in rmls:
            if len(edges)>numnode-1 and len(graph[rml[1]]['in'])>=2:
        #print rml
...
def main():
    rulefile=open('/Users/wei/Documents/folk_exp/backbone/lc/rulelist_hou','r')
    ...
    #print>>outfile,graph
    #print>>outfile,edges
    gpd=build_gpd(graph,edges,numnode,'_hou')
    outfile=open('gpd2_hou','w')
    outfile.write(cjson.encode(gpd))

def mainxxx():
    rulefile=open('/Users/wei/Documents/folk_exp/backbone/lc/rulelist_sf','r')
    tagfreqdict=open('/Users/wei/Documents/folk_exp/backbone/lc/tag_freq_dict_sf','r')
    graph,edges,numnode=construct_directed_graph_from_rules(rulefile,tagfreqdict)
    graph,edges=reduce_graph(graph,edges)
    #outfile=open('test','w')
    #print>>outfile,graph
...
def costfunc(): 
    ...
    for i in range(len(edgelist)):
        edgelist[i][-1]=(edgelist[i][2]**3)*(edgelist[i][3]-1)
        
    gpd=gpd_continue(edgelist,edges,graph,numnode)
    outfile=open('gpd_sf_cost','w')
    outfile.write(cjson.encode(gpd))
...
def test():
    ...
    graph,edges,numnode=construct_directed_graph_from_rules(rulefile,tagfreqdict)
    cur_graph=deepcopy(graph)
    if len(graph['cardinal']['in'])>=2:
        del cur_graph['baseball']['out']['cardinal']
        del cur_graph['cardinal']['in']['baseball']
        #print edge
    print shortest(cur_graph,'baseball','cardinal')
    pass
