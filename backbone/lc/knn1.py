...
for key,value in user_tag_dict_p.iteritems():
    ...
    length1=0
    for tag,num in value.iteritems():
        length1+=num*num
    length1=length1**0.5
    print length1
    for key1,value1 in user_tag_dict_all.iteritems():
        ...
        cos=0
        length2=0
        for tag,num in value1.iteritems():
            length2+=num*num
        length2=length2**0.5
        ...
        #print length2
        for tag,num in value.iteritems():
            cos+=num*value1.get(tag,0)
        cos=cos/(length1*length2)
        #print cos, 'cos'
        similarity[key1]=cos
    #print similarity
    top10=sorted(similarity.items(),key=itemgetter(1),reverse=1)[1:50]
    #print key,top10
    ...
    for users,sim in top10:
        ...
        top10agg={ k: top10agg.get(k, 0) + sim*y.get(k, 0) for k in set(top10agg) | set(y) }
