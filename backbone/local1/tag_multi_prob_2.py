...
for key,value in tagdict.iteritems():
    ...
    if common>=3 and freq1>=freq2:
        ...
        dist1[int(x1*100)]+=1
        dist2[int(x2*100)]+=1
        relation[key1][key2]=[x1,x2]
        if x2>0.9 and x1>0.88 and key1!=key2 and ' ' not in key1:
        ...
        x2=common/freq1
        if x2>0.9 and x1>0.88 and key1!=key2 and ' ' not in key2:
            #print 'remove second',key1,key2,x1,common,freq1,freq2
            removelist.append(key2)
        relation[key2][key1]=[x1,x2]
